...
if __name__ == '__main__':
    ...
    polygon_blue_value_1 = cv2.fillPoly(
        mask_image_temp, [ndarray_pts_blue], color=1)
    ...
    polygon_yellow_value_2 = cv2.fillPoly(
        mask_image_temp, [ndarray_pts_yellow], color=2)
    ...
    polygon_gray_value_3 = cv2.fillPoly(
        mask_image_temp, [ndarray_pts_gray], color=3)
    ...
    polygon_mask_blue_and_yellow = polygon_blue_value_1 + \
        polygon_yellow_value_2 + polygon_gray_value_3

    # 缩小尺寸，1920x1080->960x540
    polygon_mask_blue_and_yellow = cv2.resize(
        polygon_mask_blue_and_yellow, (960, 540))
    ...
    # 黄 polygon图片
    yellow_image = np.array(polygon_yellow_value_2 *
                            yellow_color_plate, np.uint8)
    ...
    while True:
        ...
        if len(list_bboxs) > 0:
            ...
            for item_bbox in list_bboxs:
                ...
                if polygon_mask_blue_and_yellow[y, x] == 1:
                    ...
                    if track_id in list_overlapping_gray_polygon:
                        leave_count += 1
                        print(
                            f'类别: {label} | id: {track_id} | 从上方离开区域 | 离开人数: {leave_count} | 图中人物：{list_overlapping_gray_polygon}')
                    ...
                    if track_id in list_overlapping_gray_polygon:
                        ...
                        leave_count += 1
                        print(
                            f'类别: {label} | id: {track_id} | 从下方离开区域 | 离开人数: {leave_count} | 图中人物：{list_overlapping_gray_polygon}')
                        list_overlapping_gray_polygon.remove(track_id)
                        pass
                    else:
                        # 无此 track_id，不做其他操作
                    ...
                    if track_id in list_overlapping_blue_polygon:
                        enter_count += 1
                        print(
                            f'类别: {label} | id: {track_id} | 从上方进入区域 | 进入人数: {enter_count} | 图中人物：{list_overlapping_gray_polygon}')
                        list_overlapping_blue_polygon.remove(track_id)
                    elif track_id in list_overlapping_yellow_polygon:
                        enter_count += 1
                        print(
                            f'类别: {label} | id: {track_id} | 从下方进入区域 | 进入人数: {enter_count} | 图中人物：{list_overlapping_gray_polygon}')
            ...
            # ----------------------清除无用id----------------------
            list_overlapping_all = list_overlapping_yellow_polygon + \
                list_overlapping_blue_polygon
